...
def requeststudent(request,id):
    all_id = [x.student_id for x in ClassJoined.objects.filter(class_join_id=id,is_accept=True)]
    reqeust_id = [x.student_id for x in ClassJoined.objects.filter(class_join_id=id, is_accept=False)]
    context = {
...
def requestAccept(request,class_id,student_id):
    classJoin = ClassJoined.objects.get(class_join_id=class_id,student_id=student_id)
...
def comment(request):
    if request.method=='GET':
        ...
        try:
            ...
            s = Comment(comment=comment,stream_id=stream_id,user_id=request.user.id)
            s.save()
            return HttpResponseRedirect(request.META.get('HTTP_REFERER', '/'))
        except:
            messages.add_message(request, messages.ERROR, 'some error occured')
