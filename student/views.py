...
class StudentView(View):
    ...
    def get(self,request,*args,**kwargs):
        all_id = [x.class_join_id for x in ClassJoined.objects.filter(student_id=getLogedInStudentId(request.user.id),is_accept=True)]
...
def searchclass(reqeust):
    if reqeust.method=='GET':
        ...
        c = ClassJoined.objects.filter(student_id=getLogedInStudentId(reqeust.user.id)).only('class_join_id')
...
def classJoin(request,class_id):
    if request.method=='GET':
        try:
            ...
            c.save()
            messages.add_message(request,messages.SUCCESS,'Your request to join the class has been sent')
            return redirect('student_dashbaord')
        except Exception as e:
            messages.add_message(request, messages.ERROR, 'sorry we could not sent your request right now : '+str(e))
            return redirect('student_dashbaord')
    else:
        return redirect('student_dashbaord')