...
def add_teacher_form(request):
    ...
    if request.method == 'GET':
        ...
        user = Account(email = email,password= make_password(password), is_teacher = True, is_staff =True)
        user.save()
        EmailAuto(recipient_list= [email,], html_message = html)
        teacher = Teacher(user = user, username= user_name)
        teacher.save()
        messages.success(request,f"Teacher '{ user.email }' created successfully.  ")
