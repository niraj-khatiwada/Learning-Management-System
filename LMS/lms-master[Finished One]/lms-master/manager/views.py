...
class TeacherView(View):
    ...
    def post(self,request,*args,**kwargs):
        ...
        try:
            user = Account(email=email,password=make_password(password_),is_teacher=True)
            user.save()
            msg = f'{name}, Your account is created Please use the following credential to login into your account \n Email: {email} \n password: {password_}'
            Mail(subject="Account Creation",message=msg,recipient_list=[email,])
...
class StudentView(View):
    ...
    def post(self,request,*args,**kwargs):
        ...
        password_ = randomPassword()
        try:
            user = Account(email=email,password=make_password(password_),is_student=True)
            user.save()
            msg = f'{name}, Your account is created Please use the following credential to login into your account \n Email: {email} \n password: {password_}'
            Mail(subject="Account Creation",message=msg,recipient_list=[email,])
...
def suspend_teacher(request,user_id):
    try:
        if suspend_user(user_id):
            messages.add_message(request,messages.SUCCESS,'successfully suspended')
        else:
            messages.add_message(request, messages.ERROR, 'could not suspend right now plase try again later')
        return redirect('manage_teacher')
    except:
        messages.add_message(request, messages.ERROR, 'could not suspend right now plase try again later')
        return redirect('manage_teacher')
...
def unsuspend_teacher(request,user_id):
    try:
        if unsuspend_user(user_id):
            messages.add_message(request,messages.SUCCESS,'successfully Released')
        else:
            messages.add_message(request, messages.ERROR, 'could not release right now plase try again later')
        return redirect('manage_teacher')
    except:
        messages.add_message(request, messages.ERROR, 'could not release right now plase try again later')
        return redirect('manage_teacher')
